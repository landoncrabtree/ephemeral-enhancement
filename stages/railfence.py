...
N_RAILFENCE_CHARSET_MODES = N_CHARSET_MODES
...
def railfence_decrypt(
    ...
    if charset_mode != CHARSET_ALL:
        trans_chars, trans_positions = split_selected(cipher, charset_mode)
        if not trans_chars:
            return cipher
        decrypted = _railfence_decrypt_raw("".join(trans_chars), num_rails, offset)
        return merge_selected(cipher, trans_positions, decrypted)
